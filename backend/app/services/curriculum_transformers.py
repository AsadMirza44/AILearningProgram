...
WEEK_CONTENT = {
    ...
    "week-02": {
        ...
        "curriculum": {
            ...
            "activities": [
                {"title": "Bad prompt vs good prompt challenge", "objective": "Show that prompt quality shapes output quality.", "instructions": ["Give students weak prompts.", "Ask them to improve each using the formula.", "Compare results."], "expected_outcome": "Students see direct gains from better prompt structure.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-02", "01-prompt-repair")},
                {"title": "Prompt improvement competition", "objective": "Practice iterative improvement.", "instructions": ["Students compete to improve a weak prompt most effectively.", "Vote on the strongest revised version.", "Discuss why it works."], "expected_outcome": "Students learn prompt quality through comparison.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-02", "02-prompt-ladder")},
                ...
                {"title": "Context window compression", "objective": "Practice trimming overloaded prompts.", "instructions": ["Give teams the overloaded prompt from the example folder.", "They must compress it to under 100 words while keeping all essential context.", "Compare compressed versions and discuss what was safe to remove."], "expected_outcome": "Students can identify essential vs unnecessary context in a prompt.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-02", "03-context-compression")}
            ],
            "assignments": [
    ...
    "week-03": {
        ...
        "curriculum": {
            ...
            "concepts": [
                ...
                _concept("Precision and Recall", "Precision is the fraction of positive predictions that are actually correct. Recall is the fraction of actual positives that the model correctly identified. High precision means few false alarms. High recall means few missed cases. Both metrics matter when accuracy alone hides how a model fails.", "Students encounter precision and recall in real ML evaluation reports, job descriptions, and tools. Accuracy alone can be misleadingly high when one class dominates the dataset.", "A spam filter with high precision rarely marks real emails as spam, but with low recall it misses many actual spam messages. A medical diagnosis model needs high recall so it does not miss sick patients.", ["Spam detection trade-off", "Medical diagnosis sensitivity", "Search result relevance"], ["Thinking accuracy is always the right metric", "Assuming high precision and high recall are always both achievable easily"], ["Show the spam example to make precision and recall concrete", "Connect these metrics to fairness: low recall on a group means that group is underserved"])
            ],
            "lecture_notes": [
                {"title": "Student-Friendly Definitions", "fields": {"definitions": [
            ...
            "activities": [
                ...
                {"title": "Classroom survey data collection", "objective": "Create a mini dataset.", "instructions": ["Open the study-habits.csv file from the example folder.", "Identify the feature columns and the label column.", "Discuss: what could a model predict from this data, and where might bias appear?"], "expected_outcome": "Students connect a real dataset to ML concepts — features, labels, and fairness.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-03", "01-study-dataset")},
                ...
                {"title": "Bias in dataset discussion", "objective": "Connect fairness and data quality.", "instructions": ["Open the biased-datasets.md file from the example folder.", "For each scenario, identify who is missing, what bad outcome could follow, and one fix.", "Discuss: when should a biased system be paused rather than deployed?"], "expected_outcome": "Students recognise that bad or incomplete data creates unfair results.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-03", "03-bias-inspection")},
                {"title": "Regression estimate challenge", "objective": "Differentiate classification from numeric prediction.", "instructions": ["Open the task-cards.csv from the classification-regression folder.", "Sort each task into classification, regression, or recommendation.", "Defend two borderline choices to the class."], "expected_outcome": "Students can separate regression tasks from classification and recommendation tasks.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-03", "02-classification-regression")}
            ],
            "assignments": [
    ...
    "week-04": {
        ...
        "curriculum": {
            ...
            "concepts": [
                ...
                _concept("Calling an AI API from Python", "An AI API is a web service that accepts prompts and returns model-generated responses. Python can call an API using an SDK such as the Anthropic library in a few lines: create a client, write a prompt, call the API, and print the result. API keys must always be loaded from environment variables, never hard-coded.", "This is the bridge between understanding AI and building with it. Students who learn to call an API can integrate AI into any Python project — a study assistant, quiz generator, or summariser.", "A student writes a 15-line Python script that sends a question to Claude and prints the answer, then extends it into a personal study assistant that accepts any topic.", ["Study helper script", "Quiz question generator", "Note summariser", "Automated flashcard maker"], ["Pasting API keys directly into shared code", "Not handling errors when the API is unavailable or the key is wrong"], ["Always load API keys from environment variables, never hard-code them", "Start with small max_tokens limits to manage cost while learning", "Use the starter file in content/activity-examples/week-04/05-api-basics/"])
            ],
            "lecture_notes": [
                {"title": "Student-Friendly Definitions", "fields": {"definitions": [
            ...
            "activities": [
                ...
                {"title": "Build a simple calculator", "objective": "Connect variables, input, and output.", "instructions": ["Open starter.py from the example folder.", "Read through the code together.", "Students complete the challenge: add a modulo operation and then accept user input."], "expected_outcome": "Students see a working Python program and complete their first code edit.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-04", "01-calculator")},
                {"title": "Build a quiz app", "objective": "Use lists, conditions, and output.", "instructions": ["Open starter.py from the quiz-app folder.", "Walk through the list of questions and the loop structure.", "Students complete the TODO: add case-insensitive comparison, add 2 more questions, then tackle the score challenge."], "expected_outcome": "Students build a working quiz app with a list, loop, conditions, and score.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-04", "02-quiz-app")},
                {"title": "Build a rule-based chatbot", "objective": "Apply decision logic.", "instructions": ["Open starter.py from the rule-chatbot folder.", "The bot has 2 conditions built in.", "Students add at least 5 more conditions: schedule, grades, absent, help, library."], "expected_outcome": "Students build a working multi-branch chatbot and see how rule-based logic differs from AI generation.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-04", "03-rule-chatbot")},
                {"title": "Automate a study schedule", "objective": "Turn a repetitive task into simple logic.", "instructions": ["Open starter.py from the study-schedule folder.", "Students complete the print statement inside the loop.", "Challenge: sort tasks by duration, then group by subject."], "expected_outcome": "Students use a list of dicts, a function, and enumerate() to build a working schedule generator.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-04", "04-study-schedule")},
                ...
                {"title": "Call an AI API from Python", "objective": "Connect Python code to a live AI model via the Anthropic SDK.", "instructions": ["Open starter.py from the api-basics folder.", "Walk through the 4 steps: create client, write prompt, call API, print result.", "Students run it with the class API key, change the prompt, then complete Challenge 3: accept user input."], "expected_outcome": "Students send their first API call and understand client, model, max_tokens, and the prompt-response loop.", "estimated_time": "25 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-04", "05-api-basics")}
            ],
            "assignments": [
    ...
    "week-05": {
        ...
        "curriculum": {
            ...
            "activities": [
                {"title": "Paper-based RAG simulation", "objective": "Experience grounded answering without code.", "instructions": ["Use the school-faq.md knowledge base from the example folder as your trusted source.", "Answer 3 questions first from memory, then answer them again by retrieving the right section from the FAQ.", "Compare quality and confidence between the memory-only and grounded answers."], "expected_outcome": "Students directly experience how grounding changes answer reliability.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-05", "01-knowledge-base")},
                {"title": "Design a RAG for the class file system", "objective": "Apply retrieval ideas to a realistic school document setup.", "instructions": ["Open the filesystem-template.md from the example folder.", "Fill in: knowledge sources, chunking strategy, retrieval rules, citation format, and freshness triggers.", "Present your design and explain one decision that surprised you."], "expected_outcome": "Students can design a filesystem-based RAG workflow with chunking, citations, and update logic.", "estimated_time": "25 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-05", "02-rag-design")},
                ...
                {"title": "MCP-style tool list design", "objective": "Think like a system designer.", "instructions": ["Open the tool-blueprint-template.md from the example folder.", "Fill in: tool names, what each does, who can use it, and whether it is allowed or restricted.", "Add at least one human review checkpoint and explain why it is needed."], "expected_outcome": "Students design a tool-connected school assistant with safety boundaries.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-05", "03-mcp-design")},
    ...
    "week-06": {
        ...
        "curriculum": {
            ...
            "activities": [
                {"title": "Agent building blocks comparison lab", "objective": "Use the comparison table in real scenarios.", "instructions": ["Open scenarios.csv from the example folder.", "For each of the 12 scenarios, label it with the best building block.", "Defend two of your answers using the Key Difference column."], "expected_outcome": "Students can justify which layer fits each scenario.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-06", "01-scenarios")},
                {"title": "Rules vs skills sorting game", "objective": "Separate always-on guidance from repeatable workflows.", "instructions": ["Open sorting-cards.csv from the example folder.", "Sort each card into Rules or Skills.", "Discuss borderline cases and rewrite one weak example."], "expected_outcome": "Students distinguish persistent guidance from procedural expertise.", "estimated_time": "15 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-06", "03-rules-vs-skills")},
                {"title": "Tool permission boundary design", "objective": "Practice safe tool access.", "instructions": ["Open sample-rules.md and sample-hook.sh from the real-examples folder to see what rules and hooks look like in practice.", "Design a school assistant: list allowed tools and forbidden tools.", "Add one human review checkpoint and write it as a rule."], "expected_outcome": "Students connect tools to safety boundaries and see real-world examples.", "estimated_time": "20 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-06", "02-real-examples")},
    ...
    "week-07": {
        ...
        "curriculum": {
            ...
            "activities": [
                ...
                {"title": "Capstone planning workshop", "objective": "Turn project ideas into structured plans.", "instructions": ["Open capstone-template.md from the example folder.", "Fill in every section: problem, users, AI features, prompts, RAG, tools, agent layers, safety, and model choice.", "Share your completed template with the class for peer feedback."], "expected_outcome": "Students produce a complete capstone plan using the structured template.", "estimated_time": "25 minutes", "example_folder_path": _program_path("content", "activity-examples", "week-07")},
